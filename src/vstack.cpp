 ...
 *
 * Стек - это структура данных, работающая по принципу последний
 ...
 * Ещё вы можете использовать итератор VVectorIterator, но
 ...
 * VVector в равной степени поддерживает STL итераторы и Java 
 ...
 * \a i должен быть в пределах вектора (т.е. 0 <= \a i < size())
 ...
 * быть сохранены в векторе без пере-выделения памяти.
 ...
 * Поиск проводится в обратном порядке от \a from. Если \a from равно -1
 ...
 * Эта функция нужна для совместимости с STL. Равносильно erase(begin())
 */
/*!
 ...
 * Выделяет память для \a size элементов. Эта функция может быть
