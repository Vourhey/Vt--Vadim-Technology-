 ...
 *
 * Класс VTime содержит время, т.е. часы, минуты, секунды и миллисекунды
 * начиная с полуночи. Он позволяет получить время с системных часов,
 * замерить промежутки и посчитать разницу. Погрешность составляет
 * всего несколько миллисекунд. Время хранится в формате 24 часа.
 *
 * Обычно для начала работы используется currentTime(), чтобы
 * получить текущее время. По умолчанию создается пустой объект.
 *
 * Получить составляющие можно через функции: hour(), minute(),
 * second() и msec(). Чтобы засечь промежутки можно использовать
 * start(), restart() и elapsed().  
 */

/*!
 * Создает нулевой объект. isNull() вернет \c true, isValid() вернет
 * \c false
 * \see isNull()
 */
VTime::VTime()
...
/*!
 * Возвращает количество миллисекунд, которые
...
/*!
 * Возвращает количество миллисекунд от этого времени 
 ...
 * число миллисекунд, которые прошли с последнего вызова 
 * start() или restart()\n\n
 * Этот метод гарантирует очень точные результаты, т.к.
...
/*!
 * Устанавливает время в соответствии с системными
 ...
 * Возвращает \c true, если текущее время не равно времени \a t.
 * Иначе \c false
 */

/*!
 * \fn bool VTime::operator<(const VTime &t) const
 * Возвращает \c true, если текущее время меньше времени \a t.
 * Иначе \c false
 */

/*!
 * \fn bool VTime::operator<=(const VTime &t) const
 * Возвращает \c true, если текущее время меньше или 
 * равно времени \a t. Иначе \c false
 */

/*!
 * \fn bool VTime::operator==(const VTime &t) const
 * Возвращает \c true, если текущее время равно времени \a t.
 * Иначе \c false
 */

/*!
 * \fn bool VTime::operator>(const VTime &t) const
 * Возвращает \c true, если текущее время больше времени \a t.
 * Иначе \c false
 */

/*!
 * \fn bool VTime::operator>=(const VTime &t) const
 * Возвращает \c true, если текущее время больше 
